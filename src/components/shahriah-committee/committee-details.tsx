...
export default function ShariahCommittee({ data }: ShahriahCommitteeProps) {
  ...
  return (
    <section className=" ">
      <div className="max-w-[1195px] mx-auto px-4 ">
        {/* Header */}
        <div className="mb-8">
          <h2 className="text-[#E74529] font-semibold text-center ltr:lg:text-left rtl:lg:text-right text-[20px] mb-4">
            {data.Shariah}
          </h2>
  ...
  {responsibilities.map((responsibility, index) => (
    <li
      key={index}
      className="leading-relaxed text-justify px-2 rtl:text-right ltr:text-left"
    >
      {responsibility}
    </li>
  ))}
