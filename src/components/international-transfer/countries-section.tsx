...
import type { Country } from "@/types/countries/countries";
...
export function CountriesSection({ data }: CountriesSectionProps) {
  ...
  // Convert Strapi format into UI format
  const mappedCountries = useMemo(() => {
    const mapped = apiCountries
      .filter((country) => country && country.attributes && country.attributes.countryName)
      .map((country) => {
        const name = country.attributes.countryName;
        const category = country.attributes.category?.data?.attributes?.name?.toLowerCase() || "";
        const flagUrl = country.attributes.countryFlag?.data?.attributes?.url || "";
